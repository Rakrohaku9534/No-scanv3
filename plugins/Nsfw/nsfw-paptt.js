...
let handler = async (m, { mufar }) => {
        if (db.data.chats[m.chat].nsfw == false && m.isGroup) return mufar.reply(m.chat, `❗ ᴏᴘᴛɪᴏɴs ɴsғᴡ ᴅɪᴄʜᴀᴛ ɪɴɪ ʙᴇʟᴜᴍ ᴅɪɴʏᴀʟᴀᴋᴀɴ ᴏʟᴇʜ ᴀᴅᴍɪɴ ɢʀᴏᴜᴘ\nketik *.on nsfw*`)
	let url = paptt[Math.floor(Math.random() * paptt.length)]
	mufar.sendFile(m.chat, url, null, 'Tch, dasar sangean', m)
}
handler.help = ['paptt']
handler.tags = ['nsfw']
