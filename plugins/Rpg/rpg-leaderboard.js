...
let handler = async (m, { mufar, args, participants, usedPrefix, command }) => {
...
${usedPrefix}${command} legendary`.trim()
  ...
  return mufar.sendFile(m.chat, imgr + 'leaderboard', '', '*––––『 LEADERBOARD 』––––*\n' + wrong, m)
...
${sortedItem.slice(page * 0, page * 5 + 5).map((user, i) => `${i + 1}.*﹙${user[type]}﹚*- ${participants.some(p => areJidsSameUser(user.jid, p.id)) ? `${mufar.getName(user.jid)} \nwa.me/` : 'ғʀᴏᴍ ᴏᴛʜᴇʀ ɢʀᴏᴜᴩ\n @'}${user.jid.split`@`[0]}`).join`\n\n`}
`.trim()
  return mufar.sendFile(m.chat, imgr + 'leaderboard', '', `*–『 GLOBAL LEADERBOARD 』–*\n` + text, m, {
