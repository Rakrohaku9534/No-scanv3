...
let handler = async (m, { mufar, command, args, usedPrefix }) => {
    ...
    mufar.lastWorkTime = mufar.lastWorkTime ? mufar.lastWorkTime : {};
    ...
    if (/kerja|work/i.test(command)) {
        ...
        if (timeNow - mufar.lastWorkTime[m.sender] < cooldown) {
            let remainingTime = cooldown - (timeNow - mufar.lastWorkTime[m.sender]);
        ...
        mufar.lastWorkTime[m.sender] = timeNow;
