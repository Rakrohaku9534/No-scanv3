...
let handler = async (m, { mufar, usedPrefix }) => {
  try {
    ...
    if (healt <= 79) {
      return mufar.reply(m.chat, `⚠️ *Minimal health harus 80* untuk berpetualang. Beli obat dengan ${usedPrefix}shop buy potion <jumlah> dan gunakan dengan ${usedPrefix}use potion <jumlah>. Untuk mendapatkan money dan potion gratis ketik *${usedPrefix}claim*`, m);
    }

    if (timeSinceLastAdventure <= timeToWait) {
      return mufar.reply(m.chat, `⌛ *Anda sudah berpetualang*, silahkan menunggu sampai *${timers}* lagi`, m);
    }
