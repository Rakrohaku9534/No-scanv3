...
[`sᴇʟʟ ᴛʀᴀsʜ`, `${usedPrefix}sell trash`]
    ...
    if (!listItems[item]) return mufar.sendFile(m.chat, image, '', text + '\n' + footer, m)
    if (command.toLowerCase() == 'buy') {
        let paymentMethod = Object.keys(listItems[item]).find(v => v in user)
        if (user[paymentMethod] < listItems[item][paymentMethod] * total) return mufar.sendFile(m.chat, imgr + 'lowcredit',
        '',
...
`.trim(), m)
        ...
        user[item] += total
        return mufar.sendFile(m.chat, imgr + 'bought', '',
...
`.trim(), m)
        ...
        return mufar.sendFile(m.chat, imgr + 'sold', '', 
