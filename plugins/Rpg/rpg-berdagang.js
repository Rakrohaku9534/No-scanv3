...
let timers = clockString(_ctimer)
  ...
  mufar.reply(m.chat, caption + '\n' + clockString(60000), m, { mentions: mufar.parseMention(caption) })
    
					setTimeout(() => {
			mufar.reply(m.chat, __caption + '\n' + `SUKSES`, m, { mentions: mufar.parseMention(__caption) })
		}, 10800000)

		setTimeout(() => {
			mufar.reply(m.chat, _caption + '\n' + clockString(10800000), m, { mentions: mufar.parseMention(_caption) })
		}, 7200000)

		setTimeout(() => {
			mufar.reply(m.chat, _caption + '\n' + clockString(7200000), m, { mentions: mufar.parseMention(_caption) })
		}, 3600000)

		setTimeout(() => {
			mufar.reply(m.chat, _caption + '\n' + clockString(3600000), m, { mentions: mufar.parseMention(_caption) })
		}, 60000)
					user.lastdagang = new Date * 1
} else mufar.reply(m.chat, `Anda Sudah Berdagang tunggu\n${timers} lagi..`, botdate, m)
}
