...
let handler = async (m, {
    ...
    if (!input) {
        ...
        let pp = await mufar.profilePictureUrl(who, "image").catch(_ => logo);
        ...
        const transformedDataResult = await Promise.all(transformedData.map(async (item, index) => ({
            ...item,
            avatar: await mufar.profilePictureUrl(dataArray[index], "image").catch(_ => logo)
        })));
...
Banned: *No*
`.trim()
            //mufar.reply(m.chat, str, m)
            await mufar.sendFile(m.chat, imgr + "INVENTORY", '', str, m, null, {
...
`.trim()
            await mufar.sendFile(m.chat, imgr + "INVENTORY", '', caption, m, null, {
...
`
            ...
            // let name = m.fromMe ? mufar.user : mufar.contacts[m.sender]
            let name = m.sender
            let sortedmoney = Object.entries(global.db.data.users).sort((a, b) => b[1].money - a[1].money)
            let sortedlevel = Object.entries(global.db.data.users).sort((a, b) => b[1].level - a[1].level)
            let sorteddiamond = Object.entries(global.db.data.users).sort((a, b) => b[1].diamond - a[1].diamond)
            ...
            let userslegendary = sortedlegendary.map(v => v[0])
            let str = `
Inventory *${await mufar.getName(name)}*
...
`.trim()
            ...
            await mufar.sendFile(m.chat, imgr + "INVENTORY", '', aineh, m, null, {
                mentions: mufar.parseMention(aineh)
            })
        }
    }
}
