...
let handler = async (m, { mufar, args, usedPrefix, DevMode }) => {
  try {
    ...
    if (t > 300000) {
      if (A > K) {
        mufar.sendFile(m.chat, kb, 'b.jpg', `*Kamu Tertangkap!* Korupsi dana bansos 🕴️💰, Denda *3 Juta* rupiah 💵`, m);
        u.money -= 3000000;
        u.lastbansos = new Date * 1;
      } else if (A < K) {
        u.money += 3000000;
        mufar.sendFile(m.chat, mb, 'b.jpg', `*Berhasil Korupsi!* Dana bansos 🕴️💰, Dapatkan *3 Juta* rupiah 💵`, m);
        u.lastbansos = new Date * 1;
      } else {
        mufar.reply(m.chat, `*Maaf!* Kamu tidak berhasil melakukan korupsi bansos dan kamu tidak akan masuk penjara karena kamu *melarikan diri* 🏃`, m);
        u.lastbansos = new Date * 1;
      }
    } else mufar.reply(m.chat, `*Sudah Melakukan Korupsi!* 💰\nHarus menunggu selama agar bisa korupsi bansos kembali\n▸ 🕓 ${timers}`, m);
  } catch (e) {
