...
const handler = async (m, { mufar }) => {
  mufar.koboy = mufar.koboy || {};

  if (mufar.koboy[m.chat]) return m.reply('Kamu sedang bermain game Koboy!');
...
Ketik *'kiri'* untuk bergerak ke kiri.`;
  ...
  let { key } = await mufar.reply(m.chat, gameState, m);
  ...
  mufar.koboy[m.chat] = {
    ...
    roomId: m.chat,
    timeout: setTimeout(() => {
      if (mufar.koboy && mufar.koboy[m.chat] && mufar.koboy[m.chat].roomId === m.chat) {
        mufar.sendMessage(m.chat, { delete: key });
        delete mufar.koboy[m.chat];
      }
    }, 60000 * 2),
  };
};
...
handler.before = async (m, { mufar }) => {
  ...
  if (!mufar.koboy[m.chat] || mufar.koboy[m.chat].roomId !== m.chat || !['kiri', 'kanan'].includes(m.text.toLowerCase())) return;

  let gameData = mufar.koboy[m.chat];
  ...
  if (m.quoted || m.quoted.id == key) {
    ...
    mufar.sendMessage(m.chat, { delete: oldkey });
    ...
    user.exp = (user.exp || 0) + earnedExps;
      delete mufar.koboy[m.chat];
      return mufar.reply(m.chat, `🎉 Selamat! @${sender.split('@')[0]} berhasil mengejar penjahat! 🎉\n\n💰 Mendapatkan uang senilai *${formatRupiah(earnedMoneys)}*\n🔼 Dapatkan *${earnedExps}* EXP\n`, m, { mentions: [sender] });
    } else if (moveCount === maxMoves) {
    mufar.sendMessage(m.chat, { delete: oldkey });
      delete mufar.koboy[m.chat];
      return mufar.reply(m.chat, `😔 Kamu kalah! @${sender.split('@')[0]} sudah mencapai batas maksimum gerakan.`, m, { mentions: [sender] });
    }
...
Ketik *'kiri'* untuk bergerak ke kiri.`;
    ...
    let msg = await mufar.relayMessage(m.chat, {
      protocolMessage: {
        key: key,
        type: 14,
        editedMessage: {
    ...
    mufar.koboy[m.chat] = Object.assign({}, mufar.koboy[m.chat], additionalData);
  }
};
