...
async function handler(m, { mufar, args, text , usedPrefix, command }) {
  //mufar.mission = mufar.mission || {}
  mufar.mission = mufar.mission ? mufar.mission : {}
  if(m.sender in mufar.mission) return m.reply("Kamu masih melakukan misi, tunggu sampai selesai!!")
  ...
  try {
    ...
    if(!(m.sender in mufar.mission)) {
      mufar.mission[m.sender] = {
        sender: m.sender,
        timeout: setTimeout(() => {m.reply('timed out');delete mufar.mission[m.sender]}, 60000),
        json
      }
      //Caption
...
- Cancel
`
      return mufar.reply(m.chat, caption, m) //SendMessage
    }
  } catch (e) {
    console.error(e)
    if(m.sender in mufar.mission) {
      let { timeout } = mufar.mission[m.sender]
      clearTimeout(timeout)
      delete mufar.mission[m.sender]
      m.reply('Rejected')
    }
  }
}
...
handler.before = async m => {
  //mufar.mission = mufar.mission || {}
  mufar.mission = mufar.mission ? mufar.mission : {}
  if(!(m.sender in mufar.mission)) return
  if(m.isBaileys) return

  let { timeout, json } = mufar.mission[m.sender]
  const cooldown = 5 * (1000 * 60) //coldown timer second
  let user = global.db.data.users[m.sender] //Get db user
