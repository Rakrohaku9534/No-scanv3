...
const xpperlimit = 1
let handler = async (m, { mufar, command, args }) => {
  ...
  if (global.db.data.users[m.sender].bank >= xpperlimit * count) {
    ...
    mufar.reply(m.chat, `Sukses menarik sebesar ${count} Money 💹`, m)
  } else mufar.reply(m.chat, `[❗] Uang dibank anda tidak mencukupi untuk ditarik sebesar ${count} money 💹`, m)
}
