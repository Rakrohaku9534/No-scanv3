...
let confirmation = {}
async function handler(m, { mufar, args, usedPrefix, command }) {
...
`.trim()
    let c = wm
    let { key } = await mufar.reply(m.chat, confirm, m, { mentions: [who] })
    confirmation[m.sender] = {
        ...
        key,
        pesan: mufar,
        timeout: setTimeout(() => (mufar.sendMessage(m.chat, { delete: key }), delete confirmation[m.sender]), 60 * 1000)
    }
}
