...
const handler = async (m, { mufar, text }) => {
  mufar.iqtest = mufar.iqtest ? mufar.iqtest : {};
  const iqtest = mufar.iqtest[m.chat];

  if (m.sender in mufar.iqtest && iqtest.state === 'playing') {
  ...
  mufar.reply(m.chat, `🧠 *Permainan Tes IQ* 🧠\n\n🌟 Mari kita mulai!\n\n🔔 Pertanyaan 1 dari ${totalQuestions}: ${currentQuestion}\n\nKetik jawaban Anda (ketik "stop" untuk menghentikan permainan):`, m);
  mufar.iqtest[m.chat].currentAnswer = currentAnswer;
  mufar.iqtest[m.chat].totalIQ = 100; // Initialize totalIQ
};
...
handler.before = async (m, { mufar }) => {
  ...
  if (nextQuestionIndex < soal.length) {
    ...
    mufar.iqtest[m.chat].currentAnswer = currentAnswer;
    mufar.iqtest[m.chat].currentQuestionIndex = nextQuestionIndex;
  } else {
