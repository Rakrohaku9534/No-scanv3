...
let handler = async (m, {
    ...
    if (user.health < 80) return mufar.sendFile(m.chat, imgr + 'lowhealth', '',
        `${htki} LOW HEALTH ${htka}\nʏᴏᴜʀ ʜᴇᴀʟᴛʜ ɪs ʙᴇʟᴏᴡ 80﹗
ᴩʟᴇᴀsᴇ ʜᴇᴀʟ ❤ ғɪʀsᴛ ᴛᴏ ᴀᴅᴠᴇɴᴛᴜʀᴇ ᴀɢᴀɪɴ.`.trim(), m)
    if (new Date - user.lastadventure <= cooldown) return mufar.sendFile(m.chat, imgr + 'cooldown', '',
...
${cmenua}`
    ...
    mufar.sendFile(m.chat, info.urlGambarOg, '',
        `${htki} ADVENTURE ${htka}\n` +
        teks.trim(), m)
