...
let handler = async (m, { mufar }) => {
  ...
  if (new Date() - global.db.data.users[m.sender].lastberbru > 500000) {
    ...
    function displayResults() {
      let res = `*🏞️ HASIL BERBURU ${mufar.getName(m.sender)} 🏞️*\n\n`;
      ...
      res += `*${author}* 🏕️`;
      mufar.reply(m.chat, res, null);
    }

    m.reply(`🏞️ *${mufar.getName(m.sender)} Sedang Berburu 🌿*\n\n`);
    ...
    let hsl = `*⏳ HASIL BERBURU ${mufar.getName(m.sender)} ⏳*\n\n`;
    let isEmpty = true;
