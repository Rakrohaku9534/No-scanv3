...
async function handler(m, { mufar, text }) {
  mufar.tambang = mufar.tambang || {};
  let user = global.db.data.users[m.sender];

  if (m.sender in mufar.tambang) {
    if (mufar.tambang[m.sender].currentArea >= mufar.tambang[m.sender].areas.length) {
...
handler.before = async m => {
  mufar.tambang = mufar.tambang || {};
  if (!(m.sender in mufar.tambang)) return;
  if (m.isBaileys) return;

  let { areas, currentArea, hasilTambang, lastTambangTime, totalReward } = mufar.tambang[m.sender];
  const cooldown = 5 * 60 * 1000; // 5 menit cooldown
  let user = global.db.data.users[m.sender];
  ...
  if (msg === 'stop') {
    ...
    if (areas[currentArea].txt === msg) {
      ...
      currentArea++;
      mufar.tambang[m.sender].currentArea = currentArea;
      mufar.tambang[m.sender].hasilTambang = hasilTambang;
      mufar.tambang[m.sender].totalReward = totalReward;
      mufar.tambang[m.sender].lastTambangTime = Date.now();
