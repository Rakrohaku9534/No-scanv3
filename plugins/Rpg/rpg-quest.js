...
async function handler(m, { mufar, text }) {
  mufar.quest = mufar.quest ? mufar.quest : {};
  let user = global.db.data.users[m.sender];
  
  if (m.sender in mufar.quest) {
    if (mufar.quest[m.sender].currentQuestion >= mufar.quest[m.sender].pertanyaan.length) {
...
handler.before = async m => {
  mufar.quest = mufar.quest ? mufar.quest : {};
  if (!(m.sender in mufar.quest)) return;
  if (m.isBaileys) return;

  let { pertanyaan, jawaban, reward, currentQuestion, jawabanBenar } = mufar.quest[m.sender];
  ...
  if (txt === 'stop') {
    ...
    currentQuestion++;
    jawabanBenar++;
    mufar.quest[m.sender].currentQuestion = currentQuestion;
    mufar.quest[m.sender].jawabanBenar = jawabanBenar;
