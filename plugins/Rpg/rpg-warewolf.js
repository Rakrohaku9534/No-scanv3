...
let handler = async (m, { mufar, command, usedPrefix, text }) => {
  const maxPlayers = 3;
  mufar.warewolf = mufar.warewolf || {};
  ...
  mufar.warewolf[groupId] = mufar.warewolf[groupId] || { players: [], started: false };
  const { players, started } = mufar.warewolf[groupId];
  ...
  if (inputcmd === "start") {
    ...
    mufar.warewolf[groupId].started = true;
