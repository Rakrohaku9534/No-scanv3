...
let handler = async (m, { mufar }) => {
    ...
    if (new Date - global.db.data.users[m.sender].lastclaim > 43200000) {
        mufar.reply(m.chat, `Kamu sudah mengclaim dan mendapatkan *1000* 💵money dan *1* 🥤potion`, m)
        user.money += 1000
        user.potion += 1
        user.lastclaim = new Date * 1
    } else mufar.reply(m.chat, `silahkan tunggu ${timers} lagi untuk bisa mengclaim lagi`, m)
}
