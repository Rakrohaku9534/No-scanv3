...
let handler = async (m, { mufar, command, text, args }) => {
    ...
    if (command === 'addmoney') {
        ...
        mufar.reply(m.chat, `Berhasil menambahkan ${moneyValue} money untuk @${user.split('@')[0]}!`, m)
        ...
        if (moneyValue > users[user].money) {
            ...
            users[user].money = 0
            mufar.reply(m.chat, `Berhasil mengurangi money untuk @${user.split('@')[0]}. Money kini menjadi 0!`, m)
        } else {
            // Removing the specified money from the user's account
            users[user].money -= moneyValue
            mufar.reply(m.chat, `Berhasil mengurangi ${moneyValue} money untuk @${user.split('@')[0]}!`, m)
        }
    }
}
