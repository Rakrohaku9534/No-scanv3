...
let handler = async (m, { mufar, command, text, args }) => {
    ...
    if (command === 'addlimit') {
        ...
        mufar.reply(m.chat, `Berhasil menambahkan ${limitValue} limit untuk @${user.split('@')[0]}!`, m)
        ...
        if (limitValue > users[user].limit) {
            ...
            users[user].limit = 0
            mufar.reply(m.chat, `Berhasil mengurangi limit untuk @${user.split('@')[0]}. Limit kini menjadi 0!`, m)
        } else {
            // Removing the specified limit from the user's account
            users[user].limit -= limitValue
            mufar.reply(m.chat, `Berhasil mengurangi ${limitValue} limit untuk @${user.split('@')[0]}!`, m)
        }
    }
}
