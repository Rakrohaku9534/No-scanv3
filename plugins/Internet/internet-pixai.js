...
let handler = async (m, { mufar, args, usedPrefix, text, command }) => {
  try {
...
`;
    ...
    mufar.sendFile(m.chat, await webp2png((await mufar.getFile(outputImg[0])).data), '', teks, m);
  } catch (error) {
    console.error(error);
    mufar.reply(m.chat, `Error: ${error}`, m);
  }
};
