...
let handler = async (m, { mufar, usedPrefix, command }) => {
	mufar.cartoon = mufar.cartoon ? mufar.cartoon : {};
	if (m.sender in mufar.cartoon)
	...
	try {
		Cartoon(img).then(async (response) => {
			if (response.message == "success") {
				...
				mufar.sendMessage(m.chat, await sticker.toMessage(), { quoted: m });
			} else {
				m.reply(
		...
		mufar.cartoon[m.sender] ? delete mufar.cartoon[m.sender] : false;
	}
};
