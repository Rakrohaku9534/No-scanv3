...
let handler = async (m, { mufar, command, args, usedPrefix }) => {
  ...
  try {
    ...
    let cap = global.wm
    mufar.sendMessage(m.chat, { video: { url: video }, caption: cap }, m)
    //mufar.sendMessage(m.chat, { audio: { url: audio }, mimetype: 'audio/mp4' }, { quoted : m })
  } catch (e) {
    console.log(e)
...
var handler = async (m, { args }) => {
    ...
    try {
        ...
        try {
            ...
            mufar.sendFile(m.chat, await hdFile.buffer(), 'video_hd.mp4', hdCaption, m);

            try {
                // Send SD video
                const sdFile = await fetch(sdMedia);
           //     mufar.sendFile(m.chat, await sdFile.buffer(), 'video_sd.mp4', sdCaption, m);
            } catch {
           ...
           //     mufar.sendFile(m.chat, await sdFile.buffer(), 'video_sd.mp4', sdCaption, m);
                ...
                mufar.sendFile(m.chat, 'facebook.mp4', 'facebook.mp4', cap, m);
            }
        }
    } catch {
