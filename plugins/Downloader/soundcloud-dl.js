...
let handler = async (m, { mufar, args }) => {
    ...
    let repl = await mufar.sendMessage(m.chat, { audio: { url: res.result.download}, mimetype: 'audio/mpeg' }, { quoted: m })
    let a = res.result.thumbnail
    await mufar.sendMessage(m.chat, { image: await mufar.resize(a, 300, 200), caption }, { quoted: repl })
	// By Chandra XD
	// Follow bang
	// TikTok : @pnggilajacn
