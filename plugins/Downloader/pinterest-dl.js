...
let handler = async (m, { mufar, text, usedPrefix, command }) => {
	...
	text.match(URL_REGEX) ?
		await mufar.sendMessage(m.chat, { [mime.split('/')[0]]: { url: res }, caption: `Succes Download: ${await shortUrl(res)}` }, { quoted: m }) :
	await mufar.sendMessage(m.chat, { image: { url: res }, caption: `Result From: ${text.capitalize()}`}, { quoted: m })
