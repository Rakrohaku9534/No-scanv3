...
let handler = async (m, { mufar, text }) => {
    ...
    for (const mp4Link of mp4Links) {
        let { key } = await mufar.sendMessage(m.chat, {
	text: caption,
      contextInfo: {
      externalAdReply: {
	...
	title: `ファイル ・「ムファル」。`,
	body: global.author,
	thumbnail: await (await mufar.getFile(thumbnail)).data,
	...
	renderLargerThumbnail: true 
        ...
        await mufar.sendMessage(m.chat, { delete: key })
            ...
            mufar.reply(m.chat, 'No MP4 video links found.', m);
        }
    } else {
        mufar.reply(m.chat, 'An error occurred while fetching the data.', m);
    }
}
