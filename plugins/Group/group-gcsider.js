...
await mufar.sendPresenceUpdate('composing', m.chat)
    var lama = 86400000 * 7
    ...
    if (total == 0) return mufar.reply(m.chat, `🚩 *There are no siders in this group.*`, m)
    mufar.reply(m.chat, `*${total}/${sum}* anggota grup *${await mufar.getName(m.chat)}* adalah sider dengan alasan :\n1. Tidak aktif selama lebih dari 7 hari\n2. Baru join tetapi tidak pernah nimbrung\n\n_“${pesan}”_\n\n*LIST SIDER :*\n${sider.map(v => '  ○ @' + v.replace(/@.+/, '' + typeof global.db.data.users[v] == "undefined" ? ' Sider ' : ' Off ' + msToDate(milliseconds * 1 - global.db.data.users[v].lastseen))).join('\n')}`, m, {
        contextInfo: {
            mentionedJid: sider
        }
    })
