...
const handler = async (m, { mufar, usedPrefix, args }) => {
const groups = Object.keys(mufar.chats)
  .filter(key => key.endsWith('@g.us'))
  .map(key => mufar.chats[key]);

  if (args.length === 0) {
    // Menampilkan daftar nama grup dengan urutan
    const list = groups.map((group, index) => `*${index + 1}.* ${group.subject}`).join('\n');
    mufar.reply(m.chat, `📋 *Daftar Nama Grup dengan Urutan:*\n\n${list}`, m);
    ...
    if (index >= 0 && index < groups.length) {
      ...
      mufar.reply(m.chat, '❌ Grup dengan urutan tersebut tidak ditemukan.', m);
    }
  } else {
    mufar.reply(m.chat, `❗ Format perintah salah. Gunakan "${usedPrefix}groups" untuk daftar grup atau "${usedPrefix}groups [nomor_urutan]" untuk informasi grup tertentu.`, m);
  }
};
