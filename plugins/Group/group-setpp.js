...
let handler = async (m, { mufar, args }) => {
  ...
  if (/image/.test(mime)) {
    ...
    await mufar.updateProfilePicture(m.chat, { url }).then(_ => m.reply('Success update profile picture'))
  } else if (args[0] && args[0].match(URL_REGEX)) {
    await mufar.updateProfilePicture(m.chat, { url: args[0] }).then(_ => m.reply('Success update profile picture'))
