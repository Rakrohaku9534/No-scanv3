...
let handler = async (m, {
    ...
    if (/image/g.test(mime) && !/webp/g.test(mime)) {
        ...
        const img = await q.download()
                ...
                await mufar.sendFile(m.chat, Buffer.from(buffer), '', '*Made by:* api.remove.bg', m)
            ...
            try {
                ...
                await mufar.sendFile(m.chat, url, 'removebg.png', '*Made by:* akuari', m)
            } catch (e) {
                let img = await webp2png(await q.download()),
                url = API('lolhuman', '/api/removebg', {
                    img
                }, 'apikey')
            await mufar.sendFile(m.chat, url, 'removebg.png', '*Made by:* lolhuman', m)
            }
        }
    } else {
