...
const handler = async (m, { mufar }) => {
  mufar.susunKalimat = mufar.susunKalimat || {};
  if (mufar.susunKalimat[m.sender]) {
  ...
  mufar.susunKalimat[m.sender] = {
    ...
    timeout: setTimeout(() => {
      if (mufar.susunKalimat[m.sender]) {
        ...
        delete mufar.susunKalimat[m.sender];
      }
    }, 60000 * 2),
  };
};
...
handler.before = async (m, { mufar }) => {
  mufar.susunKalimat = mufar.susunKalimat || {};
  if (m.isBaileys || !(m.sender in mufar.susunKalimat) || !m.text) return;

  const { originalSentence, sender, shuffledSentence, key, timeout } = mufar.susunKalimat[m.sender];
  ...
  if (isAnswerCorrect) {
    const level = mufar.susunKalimat[m.sender].level;
    mufar.reply(m.chat, `✨ *Selamat*, @${m.sender.split('@')[0]}! Kamu berhasil menyusun kalimat dengan benar pada *level ${level.toUpperCase()}*!`, m, { mentions: [m.sender] });
    mufar.sendMessage(m.chat, { delete: key });
