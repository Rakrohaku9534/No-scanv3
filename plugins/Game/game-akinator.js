...
let handler = async (m, { mufar, usedPrefix, command, text }) => {
	...
	if (text == 'end') {
		...
		if (aki.sesi) return mufar.reply(m.chat, 'Anda masih berada dalam sesi Akinator', aki.soal)
		try {
			...
			let soal = await mufar.sendMessage(m.chat, { text: txt, mentions: [m.sender] }, { quoted: m })
			aki.soal = soal
		} catch (e) {
			console.log(e)
			m.reply('Fitur Error!')
