...
const handler = async (m, {
    ...
    mufar.werewolf = mufar.werewolf ? mufar.werewolf : {};
    const ww = mufar.werewolf ? mufar.werewolf : {};
    const data = ww[chat];
    const value = args[0];
    const target = args[1];
    ...
    if (value === "create") {
        ...
        if (ww[chat].cooldown > 0) {
            if (ww[chat].time === "voting") {
                ...
                addTimer(chat, ww);
                return await run_vote(mufar, chat, ww);
            } else if (ww[chat].time === "malem") {
                clearAllVote(chat, ww);
                addTimer(chat, ww);
                return await run_malam(mufar, chat, ww);
            } else if (ww[chat].time === "pagi") {
                clearAllVote(chat, ww);
                addTimer(chat, ww);
                return await run_pagi(mufar, chat, ww);
            }
        }
        if (ww[chat].status === true)
        ...
        for (let i = 0; i < ww[chat].player.length; i++) {
            ...
            if (ww[chat].player[i].role === "werewolf") {
              ...
              "werewolf"
                    ...
                    await mufar.sendMessage(ww[chat].player[i].id, listMessage);
                    */
                    await mufar.sendMessage(ww[chat].player[i].id, {
                        text: text,
                        mentions: player,
                    });
                }
