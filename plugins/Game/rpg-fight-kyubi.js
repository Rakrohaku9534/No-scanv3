...
mufar.level = global.db.data.users[m.sender]
  ...
  m.reply(`*Pet Kamu* (🦊kyubi ${global.db.data.users[m.sender].kyubi}) ⚔️menantang 🦊kyubinya *${mufar.getName(lawan)}* (🦊kyubi ${global.db.data.users[lawan].kyubi}) lagi berkelahi.\n\nTunggu ${lamaPertarungan} menit lagi dan lihat siapa yg menang🎮.`)
  ...
  if (pointPemain > pointLawan){
    ...
    m.reply(`*${mufar.getName(m.sender)}* [${pointPemain * 10}] - [${pointLawan * 10}] *${mufar.getName(lawan)}*\n\n*Pet🦊Kamu* (kyubi ${global.db.data.users[m.sender].kyubi}) MENANG melawan 🦊kyubinya *${mufar.getName(lawan)}* (kyubi ${global.db.data.users[lawan].kyubi}) karena kyubi🦊kamu ${alasanMenang[Acakin(0,alasanMenang.length-1)]}\n\nHadiah Rp. ${hadiah.toLocaleString()}\n+1 Tiketcoin`)
  }else if (pointPemain < pointLawan){
    let denda = (pointLawan - pointPemain) * 100000
    global.db.data.users[m.sender].money -= denda
    global.db.data.users[m.sender].tiketcoin += 1
    m.reply(`*${mufar.getName(m.sender)}* [${pointPemain * 10}] - [${pointLawan * 10}] *${mufar.getName(lawan)}*\n\n*Pet🦊Kamu* (kyubi ${global.db.data.users[m.sender].kyubi}) KALAH melawan 🦊kyubinya *${mufar.getName(lawan)}* (kyubi ${global.db.data.users[lawan].kyubi}) karena pet kamu ${alasanKalah[Acakin(0,alasanKalah.length-1)]}\n\nUang kamu berkurang Rp. ${denda.toLocaleString()}\n+1 Tiketcoin`)
  }else {
