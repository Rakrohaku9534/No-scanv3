...
let handler = async (m, { mufar }) => {
	let from = m.chat
		mufar.exam = mufar.exam ? mufar.exam : {}
		if (from in mufar.exam) {
			mufar.sendMessage(m.chat, { text: 'Silahkan selesaikan permainan terlebih dahulu'}, { quoted: mufar.exam[from][0]})
			return false
		}
		let data = [
...
mufar.exam[from] = [
			await mufar.sendMessage(m.chat, { text: json.soal + '\nA. ' + json.a + '\nB. ' + json.b + '\nC. ' + json.c + '\n\n' + 'Waktumu 1 menit untuk menjawab' + '\nSoal tingkat ' + json.tingkat }, { quoted: m }),
			json.jawaban,
			setTimeout(() => {
				mufar.sendMessage(m.chat, { text: 'Waktu habis'}, { quoted: mufar.exam[from][0]})
				delete mufar.exam[from]
