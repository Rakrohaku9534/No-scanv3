...
const handler = async (m, {
    ...
    switch (selectedType) {
        case 'lastseen':
            await mufar.updateLastSeenPrivacy(selectedValue).then(() => {
            ...
            break;
        case 'online':
            await mufar.updateOnlinePrivacy(selectedValue).then(() => {
                m.reply(`Privacy ${privacyType} telah diubah menjadi ${selectedValue}`);
            }).catch(() => {
                m.reply(`Tidak dapat mengubah privacy ${privacyType}`);
            });
            break;
        case 'picture':
            await mufar.updateProfilePicturePrivacy(selectedValue).then(() => {
                m.reply(`Privacy ${privacyType} telah diubah menjadi ${selectedValue}`);
            }).catch(() => {
                m.reply(`Tidak dapat mengubah privacy ${privacyType}`);
            });
            break;
        case 'status':
            await mufar.updateStatusPrivacy(selectedValue).then(() => {
                m.reply(`Privacy ${privacyType} telah diubah menjadi ${selectedValue}`);
            }).catch(() => {
                m.reply(`Tidak dapat mengubah privacy ${privacyType}`);
            });
            break;
        case 'readreceipts':
            await mufar.updateReadReceiptsPrivacy(selectedValue).then(() => {
                m.reply(`Privacy ${privacyType} telah diubah menjadi ${selectedValue}`);
            }).catch(() => {
                m.reply(`Tidak dapat mengubah privacy ${privacyType}`);
            });
            break;
        case 'groupsadd':
            await mufar.updateGroupsAddPrivacy(selectedValue).then(() => {
                    m.reply(`Privacy ${privacyType} telah diubah menjadi ${selectedValue}`);
                })
                .catch(() => {
                    m.reply(`Tidak dapat mengubah privacy ${privacyType}`);
        ...
        case 'disappearingmode':
            await mufar.updateDefaultDisappearingMode(selectedValue).then(() => {
                    m.reply(`Privacy ${privacyType} telah diubah menjadi ${selectedValue}`);
                })
                .catch(() => {
                    m.reply(`Tidak dapat mengubah privacy ${privacyType}`);
