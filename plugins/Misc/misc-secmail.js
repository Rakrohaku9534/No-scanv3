...
let handler = async (m, {
    ...
    mufar.secmail = mufar.secmail ? mufar.secmail : {}
    let id = "secmail"
    ...
    if (lister.includes(feature)) {
        ...
        if (feature == "message") {
            if (!mufar.secmail[id]) return m.reply("Tidak ada pesan, buat email terlebih dahulu\nKetik *" + usedPrefix + command + " create*")

            try {
                let eml = await get_mails(mufar.secmail[id][2], mufar.secmail[id][3])
...
*Date* : ${v.date}
   `.trim()
        ...
        if (feature == "delete") {
            if (!mufar.secmail[id]) return m.reply("Tidak ada email yang terpakai")

            try {
                delete mufar.secmail[id]
