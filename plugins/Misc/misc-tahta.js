...
let handler = async (m, { mufar, text }) => {
    mufar.hartatahta = mufar.hartatahta ? mufar.hartatahta : {}
    if (m.chat in mufar.hartatahta) throw 'Masih ada yang sedang membuat\nTeks Harta Tahta\ndi chat ini... tunggu sampai selesai'
    else mufar.hartatahta[m.chat] = true
    m.reply('_Sedang membuat..._\n*Mohon tunggu sekitar 1 menit*')
    try {
        let img = await ht(text ? text : ':v')
        mufar.sendFile(m.chat, img, 'Harta Tahta.png', 'Made with FFmpeg', m)
    } finally {
        delete mufar.hartatahta[m.chat]
    }
}
