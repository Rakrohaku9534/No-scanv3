...
export async function handler(chatUpdate) {
	...
	try {
		...
		const isROwner = [mufar.decodeJid(global.mufar.user.id), ...global.owner.map(([number]) => number)].map(v => v.replace(/[^0-9]/g, "") + "@s.whatsapp.net").includes(m.sender)
		...
		const groupMetadata = (m.isGroup ? ((mufar.chats[m.chat] || {}).metadata || await this.groupMetadata(m.chat).catch(_ => null)) : {}) || {}
		const participants = (m.isGroup ? groupMetadata.participants : []) || []
		const user = (m.isGroup ? participants.find(u => mufar.decodeJid(u.id) === m.sender) : {}) || {} // User Data
		const bot = (m.isGroup ? participants.find(u => mufar.decodeJid(u.id) == mufar.user.jid) : {}) || {} // Your Data
		...
		for (let name in global.plugins) {
			...
			if (typeof plugin.all === "function") {
				try {
					...
					for (let [jid] of global.owner.filter(([number, _, isDeveloper]) => isDeveloper && number)) {
						let data = (await mufar.onWhatsApp(jid))[0] || {}
						if (data.exists)
			...
			let _prefix = plugin.customPrefix ? plugin.customPrefix : mufar.prefix ? mufar.prefix : global.prefix
		...
		case "remove":
			if (chat.welcome) {
				...
				for (let user of participants) {
					...
					const welcomeText = isAddAction ? (chat.sWelcome || this.welcome || mufar.welcome || `${emoji.welcome} Selamat datang, @user!`).replace("@subject", await this.getName(id)).replace("@desc", groupMetadata.desc?.toString() || "tidak diketahui") :
						(chat.sBye || this.bye || mufar.bye || `${emoji.bye} Sampai jumpa, @user!`);
			...
			break;
		case "promote":
			const promoteText = (chat.sPromote || this.spromote || mufar.spromote || `${emoji.promote} @user *telah diangkat menjadi Admin*`).replace("@user", "@" + participants[0].split("@")[0]);
			...
			break;
		case "demote":
			const demoteText = (chat.sDemote || this.sdemote || mufar.sdemote || `${emoji.demote} @user *tidak lagi menjadi Admin*`).replace("@user", "@" + participants[0].split("@")[0]);
...
export async function groupsUpdate(groupsUpdate) {
	...
	for (const groupUpdate of groupsUpdate) {
		...
		if (groupUpdate.desc) {
			text = (chats.sDesc || this.sDesc || mufar.sDesc || `*${emoji.desc} Deskripsi telah diubah menjadi*\n@desc`)
				.replace("@desc", groupUpdate.desc)
			stubType = 24
		} else if (groupUpdate.subject) {
			text = (chats.sSubject || this.sSubject || mufar.sSubject || `*${emoji.subject} Subjek telah diubah menjadi*\n@subject`)
				.replace("@subject", groupUpdate.subject)
			stubType = 21
		} else if (groupUpdate.icon) {
			text = (chats.sIcon || this.sIcon || mufar.sIcon || `*${emoji.icon} Icon telah diubah menjadi*`)
				.replace("@icon", groupUpdate.icon)
			stubType = 22
		} else if (groupUpdate.revoke) {
			text = (chats.sRevoke || this.sRevoke || mufar.sRevoke || `*${emoji.revoke} Tautan grup telah diubah menjadi*\n@revoke`)
				.replace("@revoke", groupUpdate.revoke)
			stubType = 1
		} else if (groupUpdate.announce === true) {
			text = (chats.sAnnounceOn || this.sAnnounceOn || mufar.sAnnounceOn || `*${emoji.announceOn} Grup telah ditutup!*`)
			stubType = 26
		} else if (groupUpdate.announce === false) {
			text = (chats.sAnnounceOff || this.sAnnounceOff || mufar.sAnnounceOff || `*${emoji.announceOff} Grup telah dibuka!*`)
			stubType = 26
		} else if (groupUpdate.restrict === true) {
			text = (chats.sRestrictOn || this.sRestrictOn || mufar.sRestrictOn || `*${emoji.restrictOn} Grup telah dibatasi hanya untuk peserta!*`)
			stubType = 25
		} else if (groupUpdate.restrict === false) {
			text = (chats.sRestrictOff || this.sRestrictOff || mufar.sRestrictOff || `*${emoji.restrictOff} Grup telah dibatasi hanya untuk admin!*`)
			stubType = 25
		}
	...
	if (msg) return mufar.reply(
		...
		m, {
			contextInfo: {
				...
				externalAdReply: {
					title: "Access Denied",
					thumbnail: await (await mufar.getFile("https://cdn-icons-png.flaticon.com/128/9667/9667923.png")).data
				},
			},
		}
	)
