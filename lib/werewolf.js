...
async function pagi(mufar, x, data) {
    ...
    return await mufar.sendMessage(x.room, {
        text: pagii(x),
        contextInfo: {
            externalAdReply: {
...
async function voting(mufar, x, data) {
    let row = [];
    let ment = [];
...
async function malam(mufar, x, data) {
...
async function skill(mufar, x, data) {
    skillOn(x.room, data)
        ...
        for (let i = 0; i < x.player.length; i++) {
            if (x.player[i].role === "werewolf") {
                if (x.player[i].isdead != true) {
                    ...
                    await mufar.sendMessage(x.player[i].id, {
                        text: textnya,
                        mentions: membernya,
                    });
                }
...
async function win(x, t, mufar, data) {
    ...
    if (getWinner(x.room, data).status === false || x.iswin === false) {
        ...
        return await mufar.sendMessage(sesinya, {
            text: textnya,
            contextInfo: {
                externalAdReply: {
                    title: "W E R E W O L F",
...
async function run(mufar, id, data) {
    while (getWinner(id, data).status === null) {
        ...
        if (getWinner(id, data).status != null) {
            ...
            await pagi(mufar, sesi(id, data), data);
        }
        if (getWinner(id, data).status != null) {
            win(getWinner(id, data), 1, mufar, data);
            break;
        ...
        if (getWinner(id, data).status != null) {
            ...
            await voting(mufar, sesi(id, data), data);
        }
        if (getWinner(id, data).status != null) {
            win(getWinner(id, data), 1, mufar, data);
            break;
        ...
        if (getWinner(id, data).status != null) {
            ...
            await malam(mufar, sesi(id, data), data);
        }
        if (getWinner(id, data).status != null) {
            win(getWinner(id, data), 1, mufar, data);
            break;
        } else {
            await skill(mufar, sesi(id, data), data);
        }
        if (getWinner(id, data).status != null) break;
    }
    await win(sesi(id, data), 1, mufar, data);
}

async function run_vote(mufar, id, data) {
    while (getWinner(id, data).status === null) {
        if (getWinner(id, data).status != null) {
            win(getWinner(id, data), 1, mufar, data);
            break;
...
async function run_malam(mufar, id, data) {
    while (getWinner(id, data).status === null) {
        if (getWinner(id, data).status != null) {
            win(getWinner(id, data), 1, mufar, data);
            break;
...
async function run_pagi(mufar, id, data) {
    while (getWinner(id, data).status === null) {
        if (getWinner(id, data).status != null) {
            win(getWinner(id, data), 1, mufar, data);
            break;
