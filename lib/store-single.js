...
function bind(mufar) {
  ...
  function updateNameToDb(contacts) {
    ...
    try {
      ...
      for (const contact of contacts) {
        const id = mufar.decodeJid(contact.id);
        if (!id || id === 'status@broadcast') continue;
        let chats = mufar.chats[id];
        if (!chats) chats = mufar.chats[id] = {...contact, id};
        mufar.chats[id] = {
          ...chats,
          ...({
  ...
  mufar.ev.on('contacts.upsert', updateNameToDb);
  mufar.ev.on('groups.update', updateNameToDb);
  mufar.ev.on('contacts.set', updateNameToDb);
  mufar.ev.on('chats.set', async ({chats}) => {
    try {
      for (let {id, name, readOnly} of chats) {
        id = mufar.decodeJid(id);
        if (!id || id === 'status@broadcast') continue;
        const isGroup = id.endsWith('@g.us');
        let chats = mufar.chats[id];
        if (!chats) chats = mufar.chats[id] = {id};
        chats.isChats = !readOnly;
        if (name) chats[isGroup ? 'subject' : 'name'] = name;
        if (isGroup) {
          const metadata = await mufar.groupMetadata(id).catch((_) => null);
  ...
  mufar.ev.on('group-participants.update', async function updateParticipantsToDb({id, participants, action}) {
    if (!id) return;
    id = mufar.decodeJid(id);
    if (id === 'status@broadcast') return;
    if (!(id in mufar.chats)) mufar.chats[id] = {id};
    const chats = mufar.chats[id];
    chats.isChats = true;
    const groupMetadata = await mufar.groupMetadata(id).catch((_) => null);
    if (!groupMetadata) return;
    chats.subject = groupMetadata.subject;
    chats.metadata = groupMetadata;
  });

  mufar.ev.on('groups.update', async function groupUpdatePushToDb(groupsUpdates) {
    try {
      for (const update of groupsUpdates) {
        const id = mufar.decodeJid(update.id);
        if (!id || id === 'status@broadcast') continue;
        const isGroup = id.endsWith('@g.us');
        if (!isGroup) continue;
        let chats = mufar.chats[id];
  ...
  mufar.ev.on('chats.upsert', function chatsUpsertPushToDb(chatsUpsert) {
    try {
      const {id, name} = chatsUpsert;
      if (!id || id === 'status@broadcast') return;
      mufar.chats[id] = {...(mufar.chats[id] || {}), ...chatsUpsert, isChats: true};
      const isGroup = id.endsWith('@g.us');
      if (isGroup) mufar.insertAllGroup().catch((_) => null);
    } catch (e) {
      console.error(e);
    }
  });
  mufar.ev.on('presence.update', async function presenceUpdatePushToDb({id, presences}) {
    try {
      const sender = Object.keys(presences)[0] || id;
      const _sender = mufar.decodeJid(sender);
      const presence = presences[sender]['lastKnownPresence'] || 'composing';
      let chats = mufar.chats[_sender];
      if (!chats) chats = mufar.chats[_sender] = {id: sender};
