...
export default async function (m, mufar = { user: {} }) {
  ...
  if (m.sender) {
    ...
    console.log(`   ${chalk.cyan('- Sender Name')}: ${m.name ? mufar.user.name : 'N/A'}`);
  ...
  if (typeof m?.text === 'string' && m.text) {
    ...
    if (m.mentionedJid) {
      for (const user of m.mentionedJid) {
        logMessage = logMessage.replace('@' + user.split`@`[0], chalk.blueBright('@' + await mufar.getName(user)));
      }
    }
...
console.log(chalk.redBright(`🎵 Attached Audio: ${m.msg.ptt ? '(PTT)' : '(Audio)'} - Duration: ${formattedDuration}`));
  ...
  if (m.sender) {
    ...
    console.log(chalk.blueBright(`  ${chalk.red('To')}: ${getPhoneNumber(mufar.user?.jid)}`));
...
const getContactInfo = (jid) => `   - ${getPhoneNumber(jid)} ${mufar.getName(jid) ? `~ ${mufar.getName(jid)}` : ''}`;
